...
- **Purpose:** Corporate website for BestIT Consultants, showcasing services, team, portfolio, and
  case studies.
- **Key Features:** Responsive design, SEO, accessibility, contact form (Resend integration),
...
- **Components:** Modular UI in `src/components/` (organized by domain: case-studies, common, home,
  seo, services, team, ui).
- **Data:** Static fixtures in `src/data/` (case studies, content sections, team members, etc.).
- **Services:** Data/service logic in `src/services/` (e.g., `caseStudyService.ts`,
  `image-service.ts`).
